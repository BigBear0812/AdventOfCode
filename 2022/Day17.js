...
const runSimulation = (maxRocks, jetPattern) => {
  ...
  const rocks = [
    ...
    [{x: 4, y: 2},{x: 4, y: 1},{x: 2, y: 0},{x: 3, y: 0},{x: 4, y: 0}], // reverse L
    [{x: 2, y: 3},{x: 2, y: 2},{x: 2, y: 1},{x: 2, y: 0}], // |
    [{x: 2, y: 1},{x: 3, y: 1},{x: 2, y: 0},{x: 3, y: 0}] // square
  ];
  ...
  while(rocksStopped < maxRocks){
    ...
    if(pattern){
      ...
      if(patternCache.has(patternKey)){
        ...
        if(rocksStopped % rocks.length === foundPattern.nextRock && jetUsed % jetPattern.length === foundPattern.nextJet){
...
const nextRock = (rocks, rocksStopped, jetPattern, jetUsed, grid) => {
  ...
  while(true){
    ...
    if(currentRock === null){
      ...
      currentRock = JSON.parse(JSON.stringify(rocks[rocksStopped % rocks.length]));
...
class Grid {
  ...
  checkCollision = (rock) => {
    ...
    for(let r = 0; r < rock.length && !collision; r++){
      ...
      else if(this.points.has(`${point.x},${point.y}`))
        collision = true;
    }
