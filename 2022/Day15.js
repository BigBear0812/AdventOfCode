...
const part1 = (fileContents) => {
  ...
  const segments = coveredRowSegments(
    sensorsAndBeacons,
    critRow,
...
const numCovered = (segments) => {
  ...
  for (let i = 1; i < segments.length; i++) {
    ...
    let over = Math.max(
      0,
      Math.min(highest, segments[i].right) - Math.max(lowest, segments[i].left),
    );
    if (!cons) {
      ...
      if (over > 0) highest = segments[i].right;
