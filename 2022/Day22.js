...
const part1 = (fileContents) => {
  ...
  if(finalPos.direction === '>')
    facing = 0;
  else if(finalPos.direction === 'v')
    facing = 1;
  else if(finalPos.direction === '<')
    facing = 2;
  else if(finalPos.direction === '^')
    facing = 3;
...
const convertToCube = (grid) => {
  ...
  cube.set(1, new CubeSide(grid, 50, 0,
    (pos) => {
      if(pos.direction === '^')
  ...
  cube.set(2, new CubeSide(grid, 100, 0,
    (pos) => {
      if(pos.direction === '^')
  ...
  cube.set(3, new CubeSide(grid, 50, 50,
    (pos) => {
      if(pos.direction === '^')
  ...
  cube.set(4, new CubeSide(grid, 0, 100,
    (pos) => {
      if(pos.direction === '^')
        return {x: pos.y, y: pos.x, direction: '>', side: 3};
      else if(pos.direction === '>')
  ...
  cube.set(5, new CubeSide(grid, 50, 100,
    (pos) => {
      if(pos.direction === '^')
  ...
  cube.set(6, new CubeSide(grid, 0, 150,
    (pos) => {
      if(pos.direction === '^')
  ...
  for(const instruction of instructions){
    ...
    if(distance){
      ...
      for(let s = 0; s < distance && !hitWall; s++){
        ...
        // If moving up
        if(pos.direction === '^'){
          nextSpace = cube.get(pos.side).grid[pos.y - 1] ? cube.get(pos.side).grid[pos.y - 1][pos.x] : null;
        ...
        // If moving right
          ...
          nextPos = {x: pos.x + 1, y: pos.y, direction: '>', side: pos.side};
          // Check if pos is at an edge
          if(pos.x === cube.get(pos.side).grid[pos.y].length - 1 || (nextSpace !== '.' && nextSpace !== '#')){
            nextPos = cube.get(pos.side).nextGridLocation(pos);
            nextSpace = cube.get(nextPos.side).grid[nextPos.y][nextPos.x];
          }
        }
        // If moving down
          ...
          // Check if pos is at an edge
          if(pos.y === cube.get(pos.side).grid.length - 1 || (nextSpace !== '.' && nextSpace !== '#')){
            nextPos = cube.get(pos.side).nextGridLocation(pos);
            nextSpace = cube.get(nextPos.side).grid[nextPos.y][nextPos.x];
          }
        }
    ...
    if(distance){
      ...
      for(let s = 0; s < distance && !hitWall; s++){
        ...
        // If moving right
          ...
          nextPos = {x: pos.x + 1, y: pos.y};
          // Check if pos is at an edge
          if(pos.x === grid[pos.y].length - 1 || (nextSpace !== '.' && nextSpace !== '#')){
            let x = 0;
            nextSpace = grid[pos.y][x];
            while(nextSpace !== '.' && nextSpace !== '#'){
              x++;
        ...
        // If moving down
          ...
          nextPos = {x: pos.x, y: pos.y + 1};
          // Check if pos is at an edge
          if(pos.y === grid.length - 1 || (nextSpace !== '.' && nextSpace !== '#')){
            let y = 0;
            nextSpace = grid[y][pos.x];
            while(nextSpace !== '.' && nextSpace !== '#'){
              y++;
