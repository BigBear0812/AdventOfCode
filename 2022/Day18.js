...
class Droplet {
  ...
  parseInput = (fileContents) => {
    ...
    for(const line of fileContents){
      let nums = line.split(',');
      this.coordinates.set(line, {x: parseInt(nums[0]), y: parseInt(nums[1]), z: parseInt(nums[2])});
    }
      ...
      for(let y = this.minY - 1; y <= this.maxY + 1; y++){
        for(let z = this.minZ - 1; z <= this.maxZ + 1; z++){
          ...
          if(!this.coordinates.has(`${x},${y},${z}`)){
              this.coordinates.has(`${x + 1},${y},${z}`) ? totalSurfaceArea++ : null;
              this.coordinates.has(`${x - 1},${y},${z}`) ? totalSurfaceArea++ : null;
              this.coordinates.has(`${x},${y + 1},${z}`) ? totalSurfaceArea++ : null;
              this.coordinates.has(`${x},${y - 1},${z}`) ? totalSurfaceArea++ : null;
              this.coordinates.has(`${x},${y},${z + 1}`) ? totalSurfaceArea++ : null;
              this.coordinates.has(`${x},${y},${z - 1}`) ? totalSurfaceArea++ : null;
          }
        }
      }
  ...
  exteriorSurfaceArea = () => {
    ...
    while(queue.length > 0){
      ...
      if(parseInt(currentString[0]) === this.maxX + 1 && parseInt(currentString[1]) === this.maxY + 1 && parseInt(currentString[2]) === this.maxZ + 1)
        break; 
  ...
  getValidNextSides = (current) => {
    ...
    results[0] = currentString[0] + 1 <= this.maxX + 1 ? `${currentString[0] + 1},${currentString[1]},${currentString[2]}` : null;
    results[1] = currentString[1] + 1 <= this.maxY + 1 ? `${currentString[0]},${currentString[1] + 1},${currentString[2]}` : null;
    results[2] = currentString[2] + 1 <= this.maxZ + 1 ? `${currentString[0]},${currentString[1]},${currentString[2] + 1}` : null;
    results[3] = currentString[0] - 1 >= this.minX + 1 ? `${currentString[0] - 1},${currentString[1]},${currentString[2]}` : null;
    results[4] = currentString[1] - 1 >= this.minY + 1 ? `${currentString[0]},${currentString[1] - 1},${currentString[2]}` : null;
    results[5] = currentString[2] - 1 >= this.minZ + 1 ? `${currentString[0]},${currentString[1]},${currentString[2] - 1}` : null;
