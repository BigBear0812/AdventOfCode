...
.then((fileContents) => { 
  ...
  let results = depthFirstSearch(timeLeft, startValve, input.goodValves, input.distances, []);
...
.then((map) => {
  ...
  let timeLeft = 26;
  // Use DFS to get back all of the possible paths a runner can take
  let results = depthFirstSearch(timeLeft, startValve, map.goodValves, map.distances, []);
...
const parseInput = (fileContent) => {
  ...
  const reg = new RegExp(/Valve ([A-Z][A-Z]) has flow rate=(\d+); tunnels* leads* to valves* ([A-Z, ]*)+/);
  let valves = [];
  ...
  valves.forEach(v => v.neighbors.forEach(n => graph.addEdge(v.name, n, 1)));
