...
export const run = (fileContents) => {
  ...
  for (const line of fileContents) {
    ...
    if (
      (jobs[0].start <= jobs[1].start && jobs[0].end >= jobs[1].end) ||
      (jobs[0].start >= jobs[1].start && jobs[0].end <= jobs[1].end)
    )
      //jobs[0].end >= jobs[1].end
    ...
    if (
      !(
        (jobs[0].start < jobs[1].start && jobs[0].end < jobs[1].start) ||
        (jobs[0].start > jobs[1].end && jobs[0].end > jobs[1].end)
      )
    )
      totalAnyOverlap++;
  }
