...
export const run = (fileContents) => {
  ...
  for (const line of fileContents) {
    ...
    if (matchCmd) {
      if (matchCmd[1] === "cd") {
        if (matchCmd[2] === "/") currentFolder = fileSystem;
        else if (matchCmd[2] === "..") currentFolder = currentFolder.parent;
        else
    ...
    else if (matchFolder) currentFolder.addFolder(matchFolder[1]);
  }
...
const traverseFolders = (folder, part1MaxSize) => {
  ...
  if (folder.folders.length === 0) {
    ...
    else return new TraverseFoldersResult(size, 0, allFolderSizes);
  }
  ...
  // size found and returned to find the total size of this folder.
    ...
    if (totalSize <= part1MaxSize) runningTotal += totalSize;
