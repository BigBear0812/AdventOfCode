  ...
  for (let round = 0; round < TOTALROUNDS; round++) {
    ...
    for (let m = 0; m < monkeys.length; m++) {
      ...
      while (monkey.items.length > 0) {
        ...
        if (monkey.test(item)) monkeys[monkey.trueMonkey].items.push(item);
        else monkeys[monkey.falseMonkey].items.push(item);
      }
    }
  }
...
const calcMonkeyBusiness = (monkeys) => {
  ...
  let eachMB = [];
  for (const monkey of monkeys) eachMB.push(monkey.itemsInspected);
