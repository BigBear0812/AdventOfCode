...
const simulateUntilRepeat = (moons) => {
  ...
  for(let s = 0; previous.length < 3; s++){
    ...
    if(!xFound){
      ...
      if(!xPrev.has(curState.xState))
        xPrev.set(curState.xState, s+1);
    ...
    if(!yFound){
      // If not a seen state add it to the dimension's set
      if(!yPrev.has(curState.yState))
        yPrev.set(curState.yState, s+1);
      // Otherwise a repeat has been found so add it to previous
      else{
        previous.push(s+1);
        yFound = true;
    ...
    if(!zFound){
      // If not a seen state add it to the dimension's set
      if(!zPrev.has(curState.zState))
        zPrev.set(curState.zState, s+1);
      // Otherwise a repeat has been found so add it to previous
      else{
        previous.push(s+1);
        zFound = true;
