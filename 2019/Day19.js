// Puzzle for Day 19: https://adventofcode.com/2019/day/19
// Not currently working. Will return to this in the future

export const run = (fileContents) => {
  // Parse the program in from the input file
  let program = fileContents[0].split(",").map(x => parseInt(x));

  let result1 = scanGrid(program);

  let gridOutput = result1.grid.reduce((output, row) => output + row.join('') + '\n', '\n');

  console.log(gridOutput);

  return {part1: result1.total};
}

const scanGrid = (program) => {
  // let comp = new IntCodeComputer(program);
  let grid = [];
  let total = 0
  for(let y = 0; y < 50; y++){
    let x = y;// Math.ceil(y/2)
    let row = [];// new Array(x).fill('X');
    let inBeam = false;
    for(; x >= 0; x--){
      let comp = new IntCodeComputer(program);
      let output = comp.runProgram([x, y]);
      if(output.val[0] === 0){
        row.push('.');
        if(inBeam){
          if(x > 0){
            let restOfRow = new Array(x).fill('X');
            row = row.concat(restOfRow);
          }
          break;
        }
      }
      else if(output.val[0] === 1){
        row.push('#');
        total++;
        inBeam = true;
      }
