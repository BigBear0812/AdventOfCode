...
const findNumMoves = (nodes) => {
  ...
  for (let node of wallNodes) {
    if (node.x > hightestWallX) hightestWallX = node.x;
    if (node.x < lowestWallX) lowestWallX = node.x;
  }

  // Find out which end of the wall is on the grid
  let wallEndX;
  if (hightestWallX + 1 >= goalDataX) wallEndX = hightestWallX + 1;
  if (lowestWallX - 1 > 0) wallEndX = lowestWallX - 1;
...
const printGrid = (nodeGrid) => {
  for (let y = 0; y < nodeGrid.length; y++) {
    ...
    for (let x = 0; x < nodeGrid[y].length; x++) {
      ...
      let char = ".";
      if (node.size > 100 && (node.used / node.size) * 100 >= 75) char = "#";
      else if (node.used === 0) char = "_";
      line += char;
    }
    console.log(line);
  }
