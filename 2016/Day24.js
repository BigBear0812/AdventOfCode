...
const findShortestPath = (matrix, returnToZero = false, visited = [0], current = 0, distTravelled = 0) => {
  ...
  if(visited.length === matrix.length){
    ...
    if(returnToZero)
      return distTravelled + matrix[current][0];
    else
  ...
  for(let x = 0; x < matrix[current].length; x++){
    if(x !== current && visited.indexOf(x) === -1){
      ...
      newVisited.push(x);
      allDistances.push(findShortestPath(matrix, returnToZero, newVisited, x, distTravelled + matrix[current][x]));
    }
  }
