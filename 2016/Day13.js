...
const stepsToGoal = (favNum, goal) => {
  ...
  while(states.length > 0 && numSteps === null){
    ...
    // Not at goal
      ...
      if (current.numMoves <= 49)
      locationsBelow50++;
      
      // Find next possible states
      let possibleNextLocations = [];
      possibleNextLocations.push({x: current.location.x+1, y: current.location.y});
      possibleNextLocations.push({x: current.location.x-1, y: current.location.y});
      possibleNextLocations.push({x: current.location.x, y: current.location.y+1});
      possibleNextLocations.push({x: current.location.x, y: current.location.y-1});
      ...
      for(let next of possibleNextLocations){
        ...
        if(next.x >=0 && next.y >= 0 && !seenLocations.has(`x:${next.x},y:${next.y}`)){
          // Add this spot to seen locations
          seenLocations.add(`x:${next.x},y:${next.y}`);

          // Find out if this is a wall or open space
          let comp = next.x*next.x + 3*next.x + 2*next.x*next.y + next.y + next.y*next.y;
          comp += favNum;

          let oneCount = (comp >>> 0).toString(2)
            .split('')
            .map(x => parseInt(x))
