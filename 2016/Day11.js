...
const moveTwoItems = (current, nextFloor) => {
  ...
  for (let x = 0; x < current.floors[current.elevator].microchips.length; x++) {
    for (
      let y = x + 1;
      y < current.floors[current.elevator].microchips.length;
      y++
    ) {
      ...
      if (
        (current.floors[nextFloor].generators.indexOf(micro1) !== -1 &&
        ...
        let ind1 =
          nextState.floors[current.elevator].microchips.indexOf(micro1);
        nextState.floors[current.elevator].microchips.splice(ind1, 1);
        let ind2 =
          nextState.floors[current.elevator].microchips.indexOf(micro2);
  ...
  for (let x = 0; x < current.floors[current.elevator].generators.length; x++) {
    for (
      let y = x + 1;
      y < current.floors[current.elevator].generators.length;
      y++
    ) {
      ...
      if (
        (current.floors[nextFloor].microchips.indexOf(gen1) !== -1 &&
          current.floors[nextFloor].microchips.indexOf(gen2) !== -1) ||
        current.floors[nextFloor].microchips.length <=
          current.floors[nextFloor].generators.length
      ) {
        // Generate this next state
        let nextState = JSON.parse(JSON.stringify(current));
...
const moveOneItem = (current, nextFloor) => {
  ...
  for (let x = 0; x < current.floors[current.elevator].microchips.length; x++) {
    ...
    if (
      current.floors[nextFloor].generators.indexOf(micro) !== -1 ||
      current.floors[nextFloor].generators.length === 0
    ) {
      // Generate this next state
      let nextState = JSON.parse(JSON.stringify(current));
  ...
  for (let x = 0; x < current.floors[current.elevator].generators.length; x++) {
    ...
    // generators for each of the microchips on the next floor
    if (
      current.floors[nextFloor].microchips.indexOf(gen) !== -1 ||
      current.floors[nextFloor].microchips.length <=
        current.floors[nextFloor].generators.length
    ) {
      // Generate this next state
...
const parseInput = (fileContents) => {
  ...
  let reg = new RegExp(
    /The (first|second|third|fourth) floor contains [ and]*([A-z]+-compatible microchip|[A-z]+ generator)*[, and]*([A-z]+-compatible microchip|[A-z]+ generator)*[, and]*([A-z]+-compatible microchip|[A-z]+ generator)*[, and]*([A-z]+-compatible microchip|[A-z]+ generator)*[, and]*([A-z]+-compatible microchip|[A-z]+ generator)*/,
  );
