...
export const run = (fileContents) => { 
  ...
  for(let address of addresses){
    ...
    for(let x = 0; x < address.hypernetSequences.length && !hypernetSequencesTls; x++){
    ...
    if(sequenceTls && !hypernetSequencesTls)
      supportsTls++;
...
const parseInput = (fileContents) => {
  ...
  let reg = new RegExp(/([a-z]+)\[([a-z]+)\]([a-z]+)\[*([a-z]+)*\]*([a-z]+)*\[*([a-z]+)*\]*([a-z]+)*/);
  let results = [];
