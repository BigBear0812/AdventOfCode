...
const processStep = (bots, outputs) => {
  ...
  for(let name of willMove){
    ...
    if(bot.lowType === 'output'){
      ...
      let output;
      if(outputs.has(bot.low))
        output = outputs.get(bot.low);
      else
        output = [];
    ...
    if(bot.highType === 'output'){
      // Check if the output bin exists and if not create a new array for it
      let output;
      if(outputs.has(bot.high))
        output = outputs.get(bot.high);
      else
        output = [];
...
const parseInput = (fileContents) => {
  ...
  let reg = new RegExp(/(value|bot) (\d+) [givesoe tlw]+ (bot|output) (\d+)(?: and high to (bot|output) (\d+))*/);
  ...
  for(let line of fileContents){
    ...
    if(matches[1] === 'value'){
      ...
      if(bots.has(botName))
        ...
        bot = JSON.parse(JSON.stringify(baseBot));
