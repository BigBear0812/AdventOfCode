...
const combineBlockList = (blockList) => {
  ...
  for(let block of blockList){
    ...
    for(let x = 0; x < combinedList.length && !matchFound; x++){
      ...
      if(combinedList[x].low <= block.low && combinedList[x].high >= block.high ){
      ...
      else if(combinedList[x].low >= block.low && combinedList[x].high <= block.high){
      ...
      else if(combinedList[x].low <= block.low && combinedList[x].high >= block.low && combinedList[x].high <= block.high){
        combinedList[x].high = block.high;
        matchFound = true;
      }
      // Block high value is contained in the combined list block but not the low value
      else if(combinedList[x].low <= block.high && combinedList[x].high >= block.high && combinedList[x].low >= block.low){
        combinedList[x].low = block.low;
        matchFound = true;
      }
