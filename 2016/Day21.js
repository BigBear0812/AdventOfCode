  ...
  for(let step of orderedSteps){
    ...
    // Rotate command
      ...
      // If unscrambling reverse the direction
        ...
        direction = unscramble ? (step.direction  === 'left' ? 'right' : 'left') : step.direction;
      }
...
const parseInput = (fileContents) => {
  // Swap regex
  let regSwap = new RegExp(/(swap) (position|letter) (\d+|[a-z]+) with (position|letter) (\d+|[a-z]+)/);
  // Reverse regex
  let regRev = new RegExp(/(reverse) positions (\d+) through (\d+)/);
  // Rotate regex
  let regRot = new RegExp(/(rotate) (right|left|based) (?:on position of letter )*(\d+|[a-z]+)/);
  // Move regex
  ...
  for(let line of fileContents){
    ...
    if(matchesSwap){
      steps.push({
        ...
        type: matchesSwap[2],
        first: matchesSwap[2] === 'position' ? parseInt(matchesSwap[3]) : matchesSwap[3],
        last: matchesSwap[4] === 'position' ? parseInt(matchesSwap[5]) : matchesSwap[5],
      });
    }
    // Reverse command
    else if(matchesRev){
    ...
    // Rotate command
      ...
      steps.push({
        ...
        value: matchesRot[2] === 'based' ? matchesRot[3] : parseInt(matchesRot[3])
      })
    }
    // Move command
    else if(matchesMove){
