...
export const run = async(fileContents) => {
  ...
  for(let line of fileContents){
    ...
    let point1 = {x: parseInt(matches[1]), y: parseInt(matches[2]), z: parseInt(matches[3])};
    let velocity = {x: parseInt(matches[4]), y: parseInt(matches[5]), z: parseInt(matches[6])};
    // Calculate the next point
    let point2 = {x: point1.x + velocity.x, y: point1.y + velocity.y, z: point1.z + velocity.z};
  ...
  for(let l1 = 0; l1 < lines.length; l1++){
    for(let l2 = l1+1; l2 < lines.length; l2++){
      ...
      if(det != 0){
        ...
        let interX = (lines[l2].B * lines[l1].C - lines[l1].B * lines[l2].C) / det;
        let interY = (lines[l1].A * lines[l2].C - lines[l2].A * lines[l1].C) / det;
        // Check if any of the dimensions compared to the start point of each line put this intersection in the past
        let past1X = (interX - lines[l1].points[0].x)/lines[l1].velocity.x < 0;
        let past1Y = (interY - lines[l1].points[0].y)/lines[l1].velocity.y < 0;
        let past2X = (interX - lines[l2].points[0].x)/lines[l2].velocity.x < 0;
        let past2Y = (interY - lines[l2].points[0].y)/lines[l2].velocity.y < 0;
        ...
        if(interX >= min && interX <= max && interY >= min && interY <= max && !inPast)
          inTheZone++;
      }
    }
  }
...
const part2 = async (lines) => {
  ...
  for (let l = 0; l < lines.length; l++) {
    const line = lines[l];
		...
		solver.add(t.ge(0));
		solver.add(x.add(vx.mul(t)).eq(t.mul(line.velocity.x).add(line.points[0].x)));
		solver.add(y.add(vy.mul(t)).eq(t.mul(line.velocity.y).add(line.points[0].y)));
		solver.add(z.add(vz.mul(t)).eq(t.mul(line.velocity.z).add(line.points[0].z)));
	}
  ...
  // Get the parameters for x, y, and z and sum them.
	return [model.eval(x), model.eval(y), model.eval(z)].reduce((total, val) => total += Number(val), 0);
}