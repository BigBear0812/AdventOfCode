  ...
  for(let pattern of allPatterns){
    ...
    for(let y = 0; y < pattern.length && !newCenterFound; y++){
      for(let x = 0; x < pattern[y].length && !newCenterFound; x++){
        ...
        if(currentVal === '#')
          newPattern[y] = newPattern[y].replaceAt(x, '.');
        else
          newPattern[y] = newPattern[y].replaceAt(x, '#');
        ...
        if(newPatternCenter.center !== patternCenter.center && newPatternCenter.total > 0){
          // The new center might contain the original center. Let's check that
          let finalTotal = newPatternCenter.total;
          let patternCenterPertinent = patternCenter.center.replace('[', '').replace(']', '');
...
const findCenter = (pattern) => {
  ...
  let total = 0;
  total += horizontalCenters.reduce((total, center) => total += 100 * (center.prev + 1), 0);
  total += verticalCenters.reduce((total, center) => total += (center.prev + 1), 0);
...
const checkForValidReflection = (pattern, centers) => {
  ...
  for(let center of centers){
    ...
    while(currentPrev >= 0 && currentCurr < pattern.length && currentValid){
      ...
      if(pattern[currentPrev] !== pattern[currentCurr])
        currentValid = false;
...
const findPossibleCenter = (pattern, marker) => {
  ...
  for(let l = 1; l < pattern.length; l++){
    ...
    if (prev === curr)
      output.push({prev:l-1, curr:l, marker});
  }
  return output;
}
...
const transposePattern = (pattern) => {
  ...
  patternGrid = patternGrid[0].map((_, x) => patternGrid.map((_, y) => patternGrid[y][x]));
