...
export const run = (fileContents) => {
  ...
  for (let line of fileContents) {
    ...
    let lowestZ = startBlocks.reduce(
      (lowest, current) => (lowest = current.z < lowest ? current.z : lowest),
  ...
  for (let a = 0; a < sandBlocks.length; a++) {
    if (showProgressLogging)
      console.log("Checking Block:", a, sandBlocks[a].name);
...
const canMoveDown = (restingPositions, blocks) => {
  ...
  for (let nb = 0; nb < newBlocks.length && !collision; nb++) {
    collision =
      testRestingPositions.has(JSON.stringify(newBlocks[nb])) ||
      newBlocks[nb].z <= 0;
  }
  return !collision;
};
