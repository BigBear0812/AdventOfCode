    ...
    while(queue.length > 0){
      ...
      for(let next of nextPossible){
        ...
        if(next.y >= 0 && next.y < grid.length &&
          next.x >= 0 && next.x < grid[next.y].length && 
          grid[next.y][next.x] != '#' && 
          (part2 || (next.direction == grid[next.y][next.x] || grid[next.y][next.x] == '.')) && // This checks arrow directions. Ignore this with the part 2 flag
    ...
    while(queue.length > 0){
      ...
      for(let next of nextEdges){
        if(!current.visited.has(next.node)){
          ...
          queue.push({node: next.node, visited: nextVisited, distance: current.distance + next.weight});
        }
      }
    }
