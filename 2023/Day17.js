...
const solver = (fileContents, minStraight, maxStraight) => {
    ...
    for (let x = 0; x < grid.length; x++) {
      ...
      if (y > 0) {
        ...
        for (let turn = 1; turn < maxStraight; turn++) {
          graph.addEdge(
            { x, y, direction: "^", lastTurn: turn - 1 },
            { x, y: y - 1, direction: "^", lastTurn: turn },
            grid[y - 1][x],
          );
        }
        for (let turn = minStraight; turn < maxStraight; turn++) {
          graph.addEdge(
            { x, y, direction: "<", lastTurn: turn },
            { x, y: y - 1, direction: "^", lastTurn: 0 },
            grid[y - 1][x],
          );
          graph.addEdge(
            { x, y, direction: ">", lastTurn: turn },
      ...
      if (y < grid.length - 1) {
        ...
        for (let turn = 1; turn < maxStraight; turn++) {
          graph.addEdge(
            { x, y, direction: "v", lastTurn: turn - 1 },
            { x, y: y + 1, direction: "v", lastTurn: turn },
            grid[y + 1][x],
          );
        }
        for (let turn = minStraight; turn < maxStraight; turn++) {
          graph.addEdge(
            { x, y, direction: "<", lastTurn: turn },
            { x, y: y + 1, direction: "v", lastTurn: 0 },
            grid[y + 1][x],
          );
          graph.addEdge(
            { x, y, direction: ">", lastTurn: turn },
      ...
      if (x > 0) {
        ...
        for (let turn = 1; turn < maxStraight; turn++) {
          graph.addEdge(
            { x, y, direction: "<", lastTurn: turn - 1 },
            { x: x - 1, y, direction: "<", lastTurn: turn },
            grid[y][x - 1],
          );
        }
        for (let turn = minStraight; turn < maxStraight; turn++) {
          graph.addEdge(
            { x, y, direction: "^", lastTurn: turn },
            { x: x - 1, y, direction: "<", lastTurn: 0 },
            grid[y][x - 1],
          );
          graph.addEdge(
            { x, y, direction: "v", lastTurn: turn },
            { x: x - 1, y, direction: "<", lastTurn: 0 },
            grid[y][x - 1],
          );
        }
  ...
  shortestPaths.push(
    graph.dijkstraShortestPath(
      { x: 0, y: 0, direction: ">", lastTurn: 0 },
      (current) => current.startsWith(endNode),
    ),
  );
  shortestPaths.push(
    graph.dijkstraShortestPath(
      { x: 0, y: 0, direction: "v", lastTurn: 0 },
      (current) => current.startsWith(endNode),
    ),
  );
  ...
  for (let path of shortestPaths) {
    if (shortest == null || path.distance < shortest.distance) shortest = path;
  }
