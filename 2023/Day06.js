...
const part2 = (fileContents) => {
  ...
  let time = parseInt(fileContents[0].match(/([\d\s]+)/)[1].replaceAll(' ', ''));
  let distance = parseInt(fileContents[1].match(/([\d\s]+)/)[1].replaceAll(' ', ''));
...
const part1 = (fileContents) => {
  ...
  let times = Array.from(fileContents[0].matchAll(/(\d+)/g)).map(x => parseInt(x[1]));
  let distances = Array.from(fileContents[1].matchAll(/(\d+)/g)).map(x => parseInt(x[1]));
