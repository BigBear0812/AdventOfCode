...
const part2 = (grid) => {
  ...
  for(let potGear of potentialGears){
    ...
    for (let sur of surrounding){
      ...
      let number = validNumbers.find(num => num.numLocations.find(loc => loc.y === sur.y && loc.x === sur.x));
      if (number){
    ...
    if (foundNumbers.size == 2) {
      ...
      let values = Array.from(foundNumbers)
        .map(val => {
...
const isNumPart = (numLocations, numValues, grid) => {
  ...
  let foundSymbol = Array.from(surrounding)
