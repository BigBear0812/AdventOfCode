  ...
  while(!vgHigh || !kpHigh || !gcHigh || !txHigh){
    ...
    while(queue.length > 0){
      ...
      if(currentMod){
        ...
        if(newPulses && newPulses.length > 0)
...
const createModuleMap = (fileContents) => {
  ...
  for(let line of fileContents){
    ...
    let mod;
    if(!matches[1])
      mod = new Module(matches[2]);
    else if(matches[1] == '%')
      mod = new FlipFlopModule(matches[2]);
    else if(matches[1] == '&'){
      ...
      for(let line2 of fileContents){
        ...
        if(potInputs.includes(matches[2]))
          mod.addInput(matches2[1]);
      }
    }
...
class Module{
  ...
  generatePulses(pulse, input){
    return this.outputs.map(o => { return {mod: o, pulse: 'low', from: this.name} });
  }
}
...
class FlipFlopModule extends Module{
  ...
  generatePulses(pulse, input){
    ...
    // generate a pulse based on the new setting of the state
      ...
      return this.outputs.map(o => { return {mod: o, pulse: outputPulse, from: this.name} });
    }
  }
}
...
class ConjunctionModule extends Module{
  ...
  generatePulses(pulse, input){
    ...
    if(allHigh) return this.outputs.map(o => { return {mod: o, pulse: 'low', from: this.name} });
    else return this.outputs.map(o => { return {mod: o, pulse: 'high', from: this.name} });
  }
}