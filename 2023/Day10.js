...
export const run = (fileContents) => {
  ...
  let area = Math.abs(loop.reduce((total, val, index, array) => 
    total += val.x * array[(index + 1) % array.length].y - array[(index + 1) % array.length].x * val.y
  , 0)) / 2;
...
const getNextPositions = (currentPos, grid) => {
  ...
  if(grid[currentPos.y][currentPos.x] === 'S'){
    adjacent.push({y: currentPos.y-1, x: currentPos.x, expected: ['|', 'F', '7']}); // up
    adjacent.push({y: currentPos.y+1, x: currentPos.x, expected: ['|', 'J', 'L']}); // down
    adjacent.push({y: currentPos.y, x: currentPos.x-1, expected: ['-', 'F', 'L']}); // left
    adjacent.push({y: currentPos.y, x: currentPos.x+1, expected: ['-', '7', 'J']}); // right
  }
  ...
  for(let pos of adjacent){
    if(pos.y >= 0 && pos.y < grid.length && pos.x>= 0 && pos.x < grid[pos.y].length && pos.expected.includes(grid[pos.y][pos.x]))
