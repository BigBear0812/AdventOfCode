...
export const run = (fileContents) => {
  ...
  for (
    let increase = 1;
    result2 === null || result2.army === "Infection";
    increase++
  ) {
    ...
    for (let group of currentGroups) {
      if (group.army === "Immune System") group.attackDamage += increase;
    }
...
const getResult = (groups) => {
  ...
  if (uniqArmies.length === 1) {
    ...
    let unitsRemaining = groups.reduce(
  ...
  while (!simOver(groups, previous)) {
    ...
    for (let g = 0; g < groups.length; g++) {
      ...
      let targets = groups
        ...
        .map((x) => {
          let damageReceived = x.weak.includes(group.attackType)
        ...
        .sort((a, b) => {
          ...
          if (result === 0) result = b.effectivePower - a.effectivePower;
          if (result === 0) result = b.initiative - a.initiative;
          return result;
        });
    ...
    for (let g = 0; g < groups.length; g++) {
      ...
      if (group.units <= 0 || !group.targetId) continue;
...
const simOver = (groups, previous) => {
  ...
  let result =
    groups.filter((x) => x.army === "Immune System").length === 0 ||
  ...
  let groups = fileContents
    ...
    .map((section, index) => {
      ...
      for (let x = 1; x < lines.length; x++) {
        ...
        let properties = matches[3].split(";").map((prop) => {
          if (prop === "") return null;
