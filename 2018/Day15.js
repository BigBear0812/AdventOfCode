...
export const run = (fileContents) => {
  ...
  while(!foundMin){
    ...
    if(types.length === 1 && types[0] === 'E' && data2.units.size === totalElves){
      foundMin = true;
    }
  ...
  return {part1: finalValue(data1, rounds1), part2: finalValue(data2, rounds2)}; 
}
...
const runSimulation = (data) => {
  ...
  while(!done){
    ...
    for(let u = 0; !done && u < unitsOrder.length; u++){
      ...
      let unitSurrounding = getSurroundingSpaces(data, {x: unit.x, y: unit.y});
      ...
      if(!attacked){
        ...
        data.units.forEach(val => {
          if(val.type !== unit.type){
            let surrounding = getSurroundingSpaces(data, {x: val.x, y: val.y})
              .filter(val => val.symbol === '.' && reachable.has(`${val.x},${val.y}`))
        ...
        if(inRange.length  > 0){
          ...
          if(moveSpace){
            ...
            attack(data, unit, getSurroundingSpaces(data, {x: unit.x, y: unit.y}));
            done = gameOver(data);
          }
        }
      } 
...
const attack = (data, unit, unitSurrounding) => {
  ...
  let attacks = unitSurrounding
    .filter(x => x.symbol !== '#' && x.symbol !== '.' && data.units.get(x.symbol).type !== unit.type)
...
const shortestDistance = (data, start, end) => {
  ...
  while(foundDistance === null && states.length > 0){
    ...
    .filter(space => space.symbol === '.' && !visited.has(`${space.x},${space.y}`))
    .forEach(val => {
      // If this is the end then set the found distance
      if(val.x === end.x && val.y === end.y)
        foundDistance = current.dis+1;
...
const allReachableSpaces = (data, start) => {
  ...
  while(states.length > 0){
    ...
    getSurroundingSpaces(data, {x: current.x, y: current.y})
    .filter(space => space.symbol === '.' && !reachable.has(`${space.x},${space.y}`))
    .forEach(val => {
      // Add the next possible state to the states array and the reachable map
      states.push({x: val.x, y: val.y, dis: current.dis+1});
      reachable.set(`${val.x},${val.y}`, {x: val.x, y: val.y, dis: current.dis+1});
    });
  }
...
const getSurroundingSpaces = (data, pos) => {
  ...
  let surrounding = []
  surrounding.push({symbol: data.grid[pos.y-1][pos.x], x: pos.x, y: pos.y-1}); // Up
  surrounding.push({symbol: data.grid[pos.y][pos.x-1], x: pos.x-1, y: pos.y}); // Left
  surrounding.push({symbol: data.grid[pos.y][pos.x+1], x: pos.x+1, y: pos.y}); // Right
  surrounding.push({symbol: data.grid[pos.y+1][pos.x], x: pos.x, y: pos.y+1}); // Down
  return surrounding;
}
