...
const findPart2 = (fileContents) => {
  ...
  for(let a = 0; a < fileContents.length && similar.length !== fileContents[0].length-1; a++){
    for(let b = a+1; b < fileContents.length && similar.length !== fileContents[0].length-1; b++){
...
const findPart1 = (fileContents) => {
  ...
  for(let line of fileContents){
    ...
    for(let a = 0; a < line.length; a++){
      for(let b = a+1; b < line.length; b++){
        for(let c = b+1; c < line.length; c++){
          if(line[a] === line[b] && line[b] === line[c] && !hasThree.has(line[a]))
            hasThree.add(line[a]);
        }
      }
    }
    ...
    for(let a = 0; a < line.length; a++){
      for(let b = a+1; b < line.length; b++){
        if(line[a] === line[b] && !hasThree.has(line[a]) && !hasTwo.has(line[a]))
          hasTwo.add(line[a]);
      }
    }
