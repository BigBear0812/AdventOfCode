...
const findOpCodes = (examples) => {
  ...
  for (let code = 0; code < allOps.length; code++) {
    opCodes[code] = examples
      ...
      .reduce((output, current) =>
        output.filter((out) => current.includes(out)),
      );
  }
  ...
  for (
    let c = 0;
    !opCodes.reduce(
      (output, current) => output && !Array.isArray(current),
      true,
    );
    c++
  ) {
    ...
    if (Array.isArray(currentCode)) {
      ...
      // filtered out of this code's array of operation's
        ...
        opCodes[code] = currentCode.filter(
          (current) => !confirmed.includes(current),
        );
      }
    }
  }
  ...
  let opsOrdered = opCodes.map(
    (code) => allOps.filter((op) => op.name === code)[0],
  );
  return opsOrdered;
};
...
const testExamples = (examples) => {
  ...
  for (let example of examples) {
    for (let op of allOps) {
      ...
      let applies = registers.reduce(
        (result, val, index) => result && val === example.after[index],
        true,
      );

      // If this operation applies to this example add the
      // operation name to it's opCodes array
      if (applies) example.opCodes.push(op.name);
    }
  }
};
