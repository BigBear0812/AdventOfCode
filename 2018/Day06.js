...
const findAreas = (data, zoneDistance) => {
  ...
  for (let y = 0; y < data.height; y++) {
    for (let x = 0; x < data.width; x++) {
      ...
      if (closestPoint.length === 1) areaCount[closestPoint[0]]++;

      // Find the sum of all distances and if it is less that the zone
      // distance increment the safe zone size count
      let totalDistance = allDistances.reduce(
        (total, val) => (total += val),
        0,
      );
      if (totalDistance < zoneDistance) safeZoneSize++;
    }
  }
...
const parseInput = (fileContents) => {
  ...
  for (let line of fileContents) {
    ...
    if (top === null || top > point.y) top = point.y;
    if (bottom === null || bottom < point.y) bottom = point.y;
    if (left === null || left > point.x) left = point.x;
    if (right === null || right < point.x) right = point.x;
