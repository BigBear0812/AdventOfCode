...
class Display {
  ...
  // Update edge values
  #updateEdges(point) {
    if (this.top === null || this.top >= point.y) this.top = point.y;
    if (this.bottom === null || this.bottom <= point.y) this.bottom = point.y;
    if (this.left === null || this.left >= point.x) this.left = point.x;
    if (this.right === null || this.right <= point.x) this.right = point.x;
  }
  ...
  print() {
    ...
    for (let y = this.top; y <= this.bottom; y++) {
      ...
      for (let x = this.left; x <= this.right; x++) {
        ...
        let strPoint = `${x},${y}`;
        if (strPoints.has(strPoint)) line += "#";
        else line += " ";
      }
