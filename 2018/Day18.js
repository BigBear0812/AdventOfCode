...
const singleRound = (grid) => {
  ...
  for (let y = 0; y < grid.length; y++) {
    ...
    for (let x = 0; x < grid[y].length; x++) {
      ...
      if (y - 1 >= 0) validDirections.add("ABOVE");
      if (y + 1 < grid.length) validDirections.add("BELOW");
      if (x - 1 >= 0) validDirections.add("LEFT");
      if (x + 1 < grid[y].length) validDirections.add("RIGHT");
      ...
      if (validDirections.has("ABOVE")) surrounding.push(grid[y - 1][x]);
      if (validDirections.has("ABOVE") && validDirections.has("RIGHT"))
        surrounding.push(grid[y - 1][x + 1]);
      if (validDirections.has("LEFT")) surrounding.push(grid[y][x - 1]);
      if (validDirections.has("RIGHT")) surrounding.push(grid[y][x + 1]);
      if (validDirections.has("BELOW") && validDirections.has("LEFT"))
        surrounding.push(grid[y + 1][x - 1]);
      if (validDirections.has("BELOW")) surrounding.push(grid[y + 1][x]);
      ...
      else if (current === "#" && (lumberyard < 1 || trees < 1)) line.push(".");
