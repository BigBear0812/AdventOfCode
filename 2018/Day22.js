...
export const run = (fileContents) => {
  ...
  let end = {
    x: parseInt(endMatches[1]),
    y: parseInt(endMatches[2]),
    tool: "torch",
  };
    ...
    do {
      ...
      if (x >= 0 && x <= end.x + spacing && y >= 0 && y <= end.y + spacing) {
        ...
        if ((x === 0 && y === 0) || (x === end.x && y === end.y))
          ...
          geologicIndex =
            grid.get(`${x - 1},${y}`).erosionLevel *
            grid.get(`${x},${y - 1}`).erosionLevel;
        ...
        let tools;
        if (type === 0) tools = ["climbing_gear", "torch"];
        else if (type === 1) tools = ["none", "climbing_gear"];
        else if (type === 2) tools = ["none", "torch"];

        // If this regions is in the area from the start to the end add it's type to the total risk
        if (x >= 0 && x <= end.x && y >= 0 && y <= end.y) totalRisk += type;
