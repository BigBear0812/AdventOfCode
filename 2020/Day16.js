...
export const run = (fileContents) => {
  ...
  for(let n = 0; n < nearbyTickets.length; n++){
    ...
    for(let v = 0; v < near.length; v++){
      ...
      for(let rule of rules){
        ...
        if((val >= rule.ranges[0].low && val <= rule.ranges[0].high) || 
    ...
    if(valid)
      validTickets.push(potentialTicket);
  }
  ...
  for(let x = 0; x < yourTicket.length; x++){
    ...
    for(let y = 1; y < validTickets.length && potentialNames.length !== 1; y++){
