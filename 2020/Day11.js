...
const runSim = (fileContents, part2) => {
  ...
  let gridInfo = {
    grid: fileContents.map((x) => x.split("")),
    occupiedCount: 0,
  };
  ...
  do {
    ...
    lastRoundGridString = gridInfo.grid.reduce(
    ...
    lastRoundOccupied !== gridInfo.occupiedCount &&
    lastRoundGridString !==
      gridInfo.grid.reduce((output, row) => (output += row.join("")), "")
  );
  ...
  for (let y = 0; y < grid.length; y++) {
    ...
    for (let x = 0; x < grid[y].length; x++) {
      ...
      // If the seat is filled and has either 4 or 5 occupied seats around it (depending on which part of the puzzle is being simulated) the empty it
        ...
        currentSeat === "#" &&
        ((!part2 && occupiedCount >= 4) || (part2 && occupiedCount >= 5))
      )
        row.push("L");
      // Otherwise the seat remains the same
      else {
        row.push(currentSeat);
        if (currentSeat === "#") newGridOccupiedCount++;
      }
    }
