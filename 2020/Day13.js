...
export const run = (fileContents) => {
  ...
  let busses = fileContents[1]
    .split(",")
    .filter((b) => b != "x")
    .map((b) => {
      return { busId: parseInt(b) };
    });
  let part1 = busses
    .map((x) => {
      return { busId: x.busId, timeAfter: x.busId - (arriving % x.busId) };
    })
    .sort((a, b) => {
      if (a.timeAfter < b.timeAfter) return -1;
      else if (a.timeAfter > b.timeAfter) return 1;
      else return 0;
    })[0];
  ...
  let busSchedule = fileContents[1]
    .split(",")
    .map((bus, timeOffset) => {
      return { bus, timeOffset };
    })
    .filter((x) => x.bus !== "x")
    .map((x) => {
      return { busId: parseInt(x.bus), timeOffset: x.timeOffset };
    });
