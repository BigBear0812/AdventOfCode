...
export const run = (fileContents) => {
  ...
  for (let tile of tiles) {
    for (let edge in tile.edges) {
      ...
      else tileEdgesMap.set(tile.edges[edge], [tile.num]);
    }
  }
  ...
  for (let y = 0; y < edgeLength; y++) {
    ...
    for (let x = 0; x < edgeLength; x++) {
      if (y === 0 && x === 0) {
        line.push(
          getCornerTile(
            tiles.find((val) => val.num === corners[0]),
            tileEdgesMap,
            top,
            left,
          ),
        ...
        line.push(
          getCornerTile(
            tiles.find((val) => val.num === corners[1]),
            tileEdgesMap,
            top,
            right,
          ),
        ...
        line.push(
          getCornerTile(
            tiles.find((val) => val.num === corners[2]),
            tileEdgesMap,
            bottom,
            left,
          ),
        ...
        line.push(
          getCornerTile(
            tiles.find((val) => val.num === corners[3]),
            tileEdgesMap,
            bottom,
            right,
          ),
...
const getCornerTile = (tile, tileEdgesMap, edge1, edge2) => {
  ...
  if (
    (tileEdgesMap.get(tile.top).length === 1 && edge1 !== "top") ||
  ...
  if (
    (tileEdgesMap.get(tile.left).length === 1 && edge2 !== "left") ||
...
const getEdges = (currentTileLines) => {
  ...
  let right = currentTileLines.reduce(
    (acc, val) => (acc += val.charAt(val.length - 1)),
    "",
  );
