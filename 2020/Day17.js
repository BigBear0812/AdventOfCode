...
const sim = (fileContents, threeD) => {
  ...
  for (let y = 0; y < fileContents.length; y++) {
    ...
    for (let x = 0; x < fileContents[y].length; x++) {
      ...
      if (line[x] === "#")
        threeD
          ? active.add(`{"x":${x},"y":${y},"z":0}`)
          : active.add(`{"x":${x},"y":${y},"z":0,"w":0}`);
    }
  }
  ...
  for (let r = 0; r < totalRounds; r++) {
    ...
    for (let a = 0; a < currentActive.length; a++) {
      ...
      for (let s = 0; s < surrounding.length; s++) {
        ...
        if (active.has(sKey)) activeCount++;
        else currentInactiveSet.add(sKey);
      }
    ...
    for (let i = 0; i < currentInactive.length; i++) {
      ...
      let activeCount = surrounding.reduce(
        (total, s) => (total += active.has(JSON.stringify(s)) ? 1 : 0),
        0,
      );
      if (activeCount === 3) nextActive.add(JSON.stringify(ci));
    }
