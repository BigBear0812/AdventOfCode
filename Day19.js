...
const parseInput = (fileContents) => {
  ...
  for(let line of fileContents){
    ...
    if(moreReplacements){
      ...
      let arr = [];
      if(replacements.has(matches[1]))
        arr = replacements.get(matches[1]);
      arr.push(matches[2]);
...
const stepsToMolecule = (info) => {
  ...
  let countRn = info.molecule.reduce((total, val) => total + (val === "Rn" ? 1 : 0), 0);
  let countAr = info.molecule.reduce((total, val) => total + (val === "Ar" ? 1 : 0), 0);
  let countY = info.molecule.reduce((total, val) => total + (val === "Y" ? 1 : 0), 0);
