...
const parseInput = (fileContents) => {
  const reg = new RegExp(/Blueprint (\d+): Each ore robot costs (\d+) ore. Each clay robot costs (\d+) ore. Each obsidian robot costs (\d+) ore and (\d+) clay. Each geode robot costs (\d+) ore and (\d+) obsidian./);
  const blueprints = [];
  ...
  for(const blueprint of blueprints){
    ...
    for(let t = 0; t < maxTime; t++){
      ...
      for(const state of states){
        ...
        if(state.stock.ore >= blueprint.geodeRobotOreCost && state.stock.obsidian >= blueprint.geodeRobotObsidianCost){
          newStates.push(new State(
            new Stock(state.stock.ore + state.robots.ore - blueprint.geodeRobotOreCost,
              state.stock.clay + state.robots.clay,
              state.stock.obsidian + state.robots.obsidian - blueprint.geodeRobotObsidianCost,
              state.stock.geode + state.robots.geode),
            new Robots(state.robots.ore, 
              state.robots.clay, 
        ...
        if(state.stock.ore >= blueprint.obsidianRobotOreCost && state.stock.clay >= blueprint.obsidianRobotClayCost){
          newStates.push(new State(
            new Stock(state.stock.ore + state.robots.ore - blueprint.obsidianRobotOreCost,
              state.stock.clay + state.robots.clay - blueprint.obsidianRobotClayCost,
        ...
        if(state.stock.ore >= blueprint.clayRobotOreCost){
          newStates.push(new State(
            new Stock(state.stock.ore + state.robots.ore - blueprint.clayRobotOreCost,
              state.stock.clay + state.robots.clay,
              state.stock.obsidian + state.robots.obsidian,
              state.stock.geode + state.robots.geode),
            new Robots(state.robots.ore, 
        ...
        if(state.stock.ore >= blueprint.oreRobotOreCost){
          newStates.push(new State(
            new Stock(state.stock.ore + state.robots.ore - blueprint.oreRobotOreCost,
              state.stock.clay + state.robots.clay,
              state.stock.obsidian + state.robots.obsidian,
              state.stock.geode + state.robots.geode),
            new Robots(state.robots.ore + 1, 
              state.robots.clay, 
              state.robots.obsidian, 
              state.robots.geode)
          ));
        ...
        newStates.push(new State(
          new Stock(state.stock.ore + state.robots.ore,
            state.stock.clay + state.robots.clay,
            state.stock.obsidian + state.robots.obsidian,
            state.stock.geode + state.robots.geode),
          new Robots(state.robots.ore, 
      ...
      states = newStates.sort((a, b) => b.score(maxTime, t) - a.score(maxTime, t)).slice(0, 500000);
    }
...
class State {
  ...
  score(maxTime, currentTime){
    return this.stock.geode + ((maxTime - currentTime) * this.robots.geode) * 10000000 
