  ...
  for (const line of fileContents) {
    ...
    for (const c of chars) {
      ...
      if (lastChar !== null) {
        ...
        if (!doubleLetter && lastChar === c) doubleLetter = true;
    ...
    if (numVowels >= 3 && doubleLetter && !badPair) niceTotal1++;
    ...
    for (let x = 1; x < chars.length; x++) {
      ...
      if (x >= 2) if (chars[x - 2] === chars[x]) numSeperatedMatches++;
    }

    // Checking if the line is nice
    if (hasDoublePairs && numSeperatedMatches >= 1) niceTotal2++;
  }
...
const findOccurences = (line, sub) => {
  ...
  while (lastIndexFound != -1) {
    if (lastIndexFound == null) lastIndexFound = line.indexOf(sub);
    else lastIndexFound = line.indexOf(sub, lastIndexFound + sub.length); // sub.length looks for non-overlapping recurrences. For overlapping recurrences use +1 instead
    if (lastIndexFound >= 0) {
