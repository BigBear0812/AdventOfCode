...
const parseInput = (fileContents) => {
  ...
  for (const line of fileContents) {
    ...
    if (graph.nodes.indexOf(matches[1]) === -1) graph.addNode(matches[1]);
    // If the second node mentioned is not in the graph add it
    if (graph.nodes.indexOf(matches[2]) === -1) graph.addNode(matches[2]);
...
class Graph {
  ...
  distancesToVisitAllNodes(current, totalCost, visited) {
    ...
    if (visited.length === this.nodes.length) return totalCost;
    ...
    for (let node of possibleNextNodes) {
      ...
      if (visited.indexOf(node.node) === -1) {
        ...
        results.push(
          this.distancesToVisitAllNodes(node.node, newTotal, newVisited),
        );
      }
    }
