...
export const run = (fileContents) => {
  ...
  for (let x = 0; x < fileContents.length; x++) {
    ...
    if (resultCombo) {
      let result1;
      let result3;
      if (isNaN(resultCombo[1])) result1 = resultCombo[1];
      else result1 = parseInt(resultCombo[1]);
      if (isNaN(resultCombo[3])) result3 = resultCombo[3];
      else result3 = parseInt(resultCombo[3]);
    ...
    if (resultNot) {
      let result2;
      if (isNaN(resultNot[2])) result2 = resultNot[2];
      else result2 = parseInt(resultNot[2]);
  ...
  for (let x = 0; x < fileContents.length; x++) {
    ...
    if (result) {
      let result1;
      if (isNaN(result[1])) result1 = getValue(result[1]);
      else result1 = parseInt(result[1]);
      wires[result[2]] = {
        command: result[1],
        value: result1,
      };
...
const getValue = (output) => {
  ...
  for (let x = 0; x < wire.inputs.length; x++) {
    if (!isNaN(wire.inputs[x])) continue;
    else wire.inputs[x] = getValue(wire.inputs[x]);
  }

  // Once the inputs are computed evaluate this wire and return the result
  if (wire.command === "~ ")
    return checkRange(eval(wire.command + wire.inputs[0]));
  else return checkRange(eval(wire.inputs[0] + wire.command + wire.inputs[1]));
};
