...
const leastExpensiveWin = (player, boss, hardMode) => {
  ...
  while (queue.length > 0) {
    ...
    if (current.player.mana < 53) continue;
    ...
    for (let effect of current.effects) {
      ...
      if (effect.name === "Poison") current.boss.hp -= 3;
      // If this is recharge the player gains 101 mana
      else if (effect.name === "Recharge") current.player.mana += 101;
    ...
    if (current.isPlayerTurn) {
      ...
      for (let spell of spells) {
        ...
        if (spell === "Magic Missle" && current.player.mana >= 53) {
          ...
          spell === "Shield" &&
          current.player.mana >= 113 &&
          current.effects.find((x) => x.name === "Shield") === undefined
        ) {
          temp = JSON.parse(JSON.stringify(current));
          temp.player.armor += 7;
          ...
          spell === "Poison" &&
          current.player.mana >= 173 &&
          current.effects.find((x) => x.name === "Poison") === undefined
        ) {
          temp = JSON.parse(JSON.stringify(current));
          ...
          spell === "Recharge" &&
          current.player.mana >= 229 &&
          current.effects.find((x) => x.name === "Recharge") === undefined
        ) {
          temp = JSON.parse(JSON.stringify(current));
        ...
        if (temp != null) {
          ...
          else if (temp.boss.hp <= 0) return temp.manaSpent;
        }
      }
    }
...
const spells = ["Magic Missle", "Drain", "Shield", "Poison", "Recharge"];
