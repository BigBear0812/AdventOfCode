  ...
  for(let y = 0; y < grid.length; y++){
    for(let x = 0; x < grid[y].length; x++){
      ...
      if(grid[y][x] == 'X'){
        ...
        // up
        if(up){
          possibilities.push([grid[y][x], grid[y-1][x], grid[y-2][x], grid[y-3][x]].join(''));
        }
        // up right
        if(up && right){
          possibilities.push([grid[y][x], grid[y-1][x+1], grid[y-2][x+2], grid[y-3][x+3]].join(''));
        }
        // up left
        if(up && left){
          possibilities.push([grid[y][x], grid[y-1][x-1], grid[y-2][x-2], grid[y-3][x-3]].join(''));
        ...
        // right
        if(right){
          possibilities.push([grid[y][x], grid[y][x+1], grid[y][x+2], grid[y][x+3]].join(''));
        }
        // left
        if(left){
          possibilities.push([grid[y][x], grid[y][x-1], grid[y][x-2], grid[y][x-3]].join(''));
