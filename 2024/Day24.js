...
const part2 = (data) => {
  ...
  wires.forEach((wire, key) => wires.set(key, {wire: key, type: 'INPUT', num: parseInt(key.slice(1))}));
  ...
  for(let gate of gates){
    ...
    if(gate.operation === 'XOR' 
      && (XYAddWires.includes(gate.inputs[0]) || XYAddWires.includes(gate.inputs[1]))
      && !gate.output.startsWith('z')){
      badWires.add(gate.output);
    }
  ...
  XYAddWires.filter(wire => !ZOutInputs.includes(wire) && wire !== 'z00')
  ...
  XYCarryWires.filter(wire => !ORInputs.includes(wire) && wires.get(wire).num !== 0)
    .forEach(wire => badWires.add(wire));
    ...
    for(let g = 0; g < gates.length; g++){
      ...
      for(let i = 0; i < gate.inputs.length && hasBoth; i++){
        ...
        if(!wires.has(input))
          ...
          inputVals.push(wires.get(input));
      }
...
const parseInput = (fileContents) => {
  ...
  for(let line of fileContents){
    ...
    // Otherwise parse the circuit's logic gate info
      ...
      let matches = line.match(/([A-z\d]+) ([XORAND]+) ([A-z\d]+) -> ([A-z\d]+)/);
