...
const solver = (connections) => {
  ...
  for (let connection of connections) {
    ...
    if (!graph.hasNode(connection.from)) graph.addNode(connection.from);
    if (!graph.hasNode(connection.to)) graph.addNode(connection.to);
  ...
  for (let node of nodes) {
    ...
    else nodeGroupCount.set(count, [node]);

    // If the current count is higher then the highest count update the highest count
    if (count > highestGroupCount) highestGroupCount = count;
  }
  ...
  findAllThreeComputerGroups() {
    ...
    for (let current in this.edges) {
      ...
      for (let neighbor of currentNeighborNodes) {
        ...
        let finalGroupNeighbors = this.edges[neighbor].filter(
          (edge) => currentNeighborNodes.includes(edge) && edge != current,
        );
