...
const part2 = async (map, position, visitedSet) => {
  ...
  for (let s = 0; s < WORKER_THREADS; s++) {
    ...
    else visitSegments.push(visitedArray);
  }
  ...
  await Promise.all(visitPromises).catch((error) => {
    console.log(error);
  });
  return countLoops;
};
...
const runSimulation = (map, position) => {
  ...
  while (canMove && !loopDetected) {
    ...
    // Else if the new location is an obstacle change direction
      ...
      // Up becomes right
      if (position.direction === "^") position.direction = ">";
      // Right becomes down
      else if (position.direction === ">") position.direction = "v";
      // Down becomes left
      else if (position.direction === "v") position.direction = "<";
      // Left becomes up
      else if (position.direction === "<") position.direction = "^";
    }
    // Otherwise this must be a valid space to move into
      ...
      else loopFinding.add(`${position.y},${position.x},${position.direction}`);
    }
  }
...
const parseInput = (fileContents) => {
  ...
  for (let line = 0; line < fileContents.length; line++) {
    ...
    if (index >= 0) position = { direction: "^", y: line, x: index };
...
if (!isMainThread) {
  ...
  for (let visit of visits) {
    ...
    if (!(visit.y == clonedPosition.y && visit.x == clonedPosition.x)) {
      ...
      if (output.loopDetected) totalLoops++;
    }
  }
