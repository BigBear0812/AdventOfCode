...
const part2 = (grid, result) => {
  ...
  let paths = result.shortestPaths.map((path) =>
    path.map((node) => {
...
const print = (grid, visited) => {
  ...
  for (let y = 0; y < grid.length; y++) {
    for (let x = 0; x < grid[y].length; x++) {
      ...
      if (visited.has(`${y},${x}`)) output += "O";
      // Otherwise use whatever is in the grid
      else output += grid[y][x];
    }
    output += "\n";
  }
  console.log(output);
...
const part1 = (data) => {
  ...
  return result1.shortestDistance < result2.shortestDistance
    ? result1
    : result2;
};
...
const createGraph = (grid) => {
  ...
  const getTurnDirections = (y, x, direction, grid) => {
    ...
    if (direction === "^" || direction === "v") {
      sides = [getNextSpace(y, x, "<", grid), getNextSpace(y, x, ">", grid)];
    } else if (direction === "<" || direction === ">") {
      sides = [getNextSpace(y, x, "^", grid), getNextSpace(y, x, "v", grid)];
    }
  ...
  while (intersections.length > 0) {
    ...
    if (
      !visitedIntersections.has(
        `${currentIntersection.y},${currentIntersection.x}`,
      )
    )
      visitedIntersections.add(
        `${currentIntersection.y},${currentIntersection.x}`,
      );
    else continue;
    ...
    for (let dir of directions) {
      ...
      while (!nextIntersection) {
        ...
        let sideDirections = getTurnDirections(
          currentSpace.y,
          currentSpace.x,
...
class Graph {
  ...
  addEdge(from, to, weight, reverse) {
    ...
    if (reverse) {
      ...
      let reverseFromNode = this.#key(
        from.x,
        from.y,
        this.#reverseDirection(from.direction),
      );
      let reverseToNode = this.#key(
        to.x,
        to.y,
        this.#reverseDirection(to.direction),
      );
      // If those nodes do not exist create them
      if (!this.edges[reverseFromNode]) this.edges[reverseFromNode] = [];
      if (!this.edges[reverseToNode]) this.edges[reverseToNode] = [];
