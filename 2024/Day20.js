 ...
 */
const solver = (data, shortcutMaxDistance) => {
  ...
  pathLocations.forEach((loc, distance) =>
    locationDistances.set(loc[0], distance),
  );
  ...
  for (let a = 0; a < pathLocations.length; a++) {
    for (let b = a + 1; b < pathLocations.length; b++) {
      ...
      let shortcutDistance =
        Math.abs(aLoc[1].y - bLoc[1].y) + Math.abs(aLoc[1].x - bLoc[1].x);
  ...
  return Array.from(results).reduce(
    (total, shortcutTime) =>
      (total += shortcutTime[0] >= 100 ? shortcutTime[1] : 0),
    0,
  );
};
 ...
 */
const createGraph = (fileContents) => {
