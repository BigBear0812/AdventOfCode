...
const runTuringMachine = (data) => {
  ...
  for (let x = 0; x < data.steps; x++) {
    ...
    if (ones.has(pos)) {
      ...
      if (current[1].write === 0) ones.delete(pos);
      // Move the position left or right
      if (current[1].move === "right") pos++;
      else if (current[1].move === "left") pos--;
      // Set the next state
    ...
    // Else the current position is a 0
      ...
      if (current[0].write === 1) ones.add(pos);
      // Move the position left or right
      if (current[0].move === "right") pos++;
      else if (current[0].move === "left") pos--;
      // Set the next state
