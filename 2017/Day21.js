...
const interate = (grid, rules) => {
  ...
  // Otherwise if the grid is divisible by 3
    ...
    for(let y = 1; y <= div; y++){
      for(let x = 1; x <= div; x++){
        sections.push([
          [grid[(3*y)-3][(3*x)-3], grid[(3*y)-3][(3*x)-2], grid[(3*y)-3][(3*x)-1]],
          [grid[(3*y)-2][(3*x)-3], grid[(3*y)-2][(3*x)-2], grid[(3*y)-2][(3*x)-1]],
          [grid[(3*y)-1][(3*x)-3], grid[(3*y)-1][(3*x)-2], grid[(3*y)-1][(3*x)-1]]
        ]);
      }
    }
    ...
    for(let section of sections){
      ...
      let matchFound = false;
      for(let x = 0; x < rules.threeRules.length && matchFound === false; x++){
