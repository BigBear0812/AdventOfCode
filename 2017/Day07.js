...
const findChildWeights = (towers, bottomProgram) => {
  ...
  if (found === null) {
    ...
    for (let x = 0; x < childWeights.length; x++) {
      ...
      for (let y = 0; y < childWeights.length && !matches; y++) {
        if (x !== y) matches = childWeights[x] === childWeights[y];
      }
      if (!matches) offBalanceIndex = x;
    }
    ...
    if (offBalanceIndex !== -1) {
      ...
      let correctWeight =
        offBalanceIndex + 1 === childWeights.length
          ? childWeights[offBalanceIndex - 1]
          : childWeights[offBalanceIndex + 1];
      ...
      let correctedIndividualWeight =
        towers.get(current.children[offBalanceIndex]).weight + difference;
      ...
      return {
        weight,
        found: {
          program: current.children[offBalanceIndex],
          correctedIndividualWeight,
        },
      };
    }
  }
  ...
  let weight =
    childWeights.reduce((sum, weight) => sum + weight, 0) + current.weight;
  return { weight, found };
};
...
const findBottomProgram = (towers) => {
  ...
  while (towers.length > 1) {
    ...
    for (let x = 0; x < towers.length; x++) {
      ...
      towers[x].children = towers[x].children.filter(
        (c) => towers.find((t) => t.program === c) !== undefined,
      );
    }
  }
